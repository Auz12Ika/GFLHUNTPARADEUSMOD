...
// ========================================================================
// PRELOAD HQ ARSENAL WHITELIST
// ========================================================================

[] spawn {
	
    [] call MERC_fnc_RequestHQWhitelist;
	private _time = time;
    waitUntil {
...
if (!isNil "MERC_HQ_ArsenalWhitelist") then {
    diag_log "[MERC] HQ Arsenal Whitelist Ready.";
} else {
    diag_log "[MERC] HQ Arsenal Whitelist Timeout.";
};
...
MERC_fnc_onShopBuyPressed = {
    ...
    if (_index == -1) exitWith { hint "Please select an infrastructure structure first!"; };

    private _class = _listCtrl lbData _index;
    private _price = _listCtrl lbValue _index;
    private _currentMoney = missionNamespace getVariable ["merc_money", 0];

    if (_currentMoney < _price) exitWith { hint "Construction Failed: Insufficient Contract Funds!"; };

    // 1. Ambil objek Mobile HQ yang sudah terdaftar di sistem
    private _hq = missionNamespace getVariable ["MERC_Player_HQ", objNull];
    if (isNull _hq) exitWith { hint "Construction Failed: Mobile HQ not found or not deployed!"; };
    ...
    closeDialog 0;
    hint "Structure purchased successfully! It has been placed 18 meters to the left of the Mobile HQ. Use the [LIFT] scroll menu to reposition it.";
};
...
MERC_fnc_onVehShopBuyPressed = {
    ...
    if (isNull _targetPad) exitWith { 
        hint format ["Deployment Failed: Target marker '%1' was not found on the map!", _padVarName]; 
    };
            ...
            _npc addAction [
                "<t color='#00FF00' scale='1.1' weight='bold'>Open Arsenal ($5000)</t>", 
                {
                    ...
                    if (_money < 5000) exitWith { 
                        hint "Insufficient funds! Your group needs $5,000 to unlock the Arsenal."; 
                    };
                    
                    hint "Verifying funds with Mobile HQ...";
...
MERC_fnc_attachHqActions = {
	...
	_unit addAction [
		...
		{
			private _hq = missionNamespace getVariable ["MERC_Player_HQ", objNull];
			if (isNull _hq) exitWith { hint "Error: Mobile HQ not found."; };

			private _nearVehicles = nearestObjects [getPosATL _hq, ["LandVehicle","Air","Ship"], 50];
			_nearVehicles pushBackUnique _hq;
			...
			hint format ["Logistics Successful: Rearmed %1 vehicle(s) near the Mobile HQ.", _rearmedCount];
		},
		nil, 15.45, true, true, "",
		_condDeployed
	];
    ...
    // Sub-Menu Database
    _unit addAction ["<t color='#00FF00'>[DATABASE] == SAVE PROGRESS TO PROFILE ==</t>", { hint "Select a save slot below."; }, nil, 15.4, false, true, "", _condDeployed];
    ...
    _unit addAction ["<t color='#00FFFF'>[DATABASE] == RESTORE DATA FROM PROFILE ==</t>", { hint "Select a save slot to restore."; }, nil, 15.0, false, true, "", _condDeployed];
    ...
    _unit addAction ["<t color='#FF0000'>[DATABASE] == CLEAR ARCHIVE / WIPE PROGRESS ==</t>", { hint "Select a save slot to erase."; }, nil, 14.6, false, true, "", _condDeployed];
    ...
    _unit addAction [
        ...
        {
            private _money = missionNamespace getVariable ["merc_money", 0];
            private _cultCount = missionNamespace getVariable ["MERC_cult_progress", 0];
            private _repUS = missionNamespace getVariable ["rep_USA", 0];
            private _repRU = missionNamespace getVariable ["rep_RUSSIA", 0];
...
MERC_fnc_openContractBoard = {
    ...
    lbClear _listBox;
	    ...
	    // 🆕 Tombol CULT CAMPAIGN
    private _cultBtn = _display displayCtrl 47430;
    if (!isNull _cultBtn) then {
        _cultBtn ctrlAddEventHandler ["ButtonClick", {
            // Cari data misi Cult dari pool
            private _cultData = nil;
            {
                if ((_x select 0) == "cult_main_1") exitWith { _cultData = _x; };
            } forEach (missionNamespace getVariable ["MERC_mission_contract_pool", []]);

            if (isNil "_cultData") exitWith { hint "Error: Cult mission data not found!"; };

            // Kirim ke server
            [_cultData, player] remoteExec ["MERC_fnc_serverAcceptContract", 2];
    ...
    if (count _activeMissions == 0) exitWith { hint "Error: No contracts are currently available."; };
    ...
    _acceptBtn ctrlAddEventHandler ["ButtonClick", {
        ...
        if (_selIndex < 0) exitWith { hint "Please select a mission first!"; };
...
// ========================================================================
// 8.HQ Arsenal Filter Receiver (CLIENT)
// ========================================================================
...
MERC_fnc_HQwhitelist  = {
    ...
    MERC_HQ_ArsenalWhitelist = +_whitelist;

    diag_log format [
        "[HQ Arsenal] Filter updated. W:%1 M:%2 I:%3 B:%4",
        count (MERC_HQ_ArsenalWhitelist select 0),
        count (MERC_HQ_ArsenalWhitelist select 1),
        count (MERC_HQ_ArsenalWhitelist select 2),
        count (MERC_HQ_ArsenalWhitelist select 3)
    ];
};
