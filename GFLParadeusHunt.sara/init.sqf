...
[] execVM "InsurgencyFunction\Mission\fn_cultBrainWatcher.sqf";
...
if (isServer) then {
	...
	MERC_fnc_ServerPayArsenal = {
		...
		if (_currentMoney < _price) exitWith {
			["Access Denied: Insufficient Group Contract Funds!"] remoteExec ["hint", _player];
		};
		...
		[] remoteExecCall ["MERC_fnc_OpenArsenalLocal", _player];
	...
	MERC_fnc_ServerBuyVehicle = {
		...
		if (isNull _targetPad) exitWith {
			[format ["Deployment Failed: Target marker '%1' was not found!", _padVarName]] remoteExec ["hint", _player];
		};
	...
	*/
    [] spawn {
        ...
        while {true} do {
            ...
            private _transition = if (_firstRun) then {0} else {1800};
            _firstRun = false;
            ...
            switch (true) do {
                ...
                // 45% - Clear
                case (_roll < 45): {
                    _overcast = 0.10 + random 0.15;
                    _rain = 0;
                    _fog = random 0.01;
                    _windMin = 1;
                    _windMax = 3;
                    _duration = 2700 + random 1800;
                };

                // 25% - Cloudy
                case (_roll < 70): {
                    _overcast = 0.35 + random 0.25;
                    _rain = 0;
                    _fog = random 0.03;
                    _windMin = 3;
                    _windMax = 5;
                    _duration = 2400 + random 1500;
                };

                // 15% - Drizzle
                case (_roll < 85): {
                    _overcast = 0.65 + random 0.15;
                    _rain = 0.15 + random 0.20;
                    _fog = 0.03 + random 0.05;
                    _windMin = 5;
                    _windMax = 8;
                    _duration = 1800 + random 900;
                };

                // 10% - Rain
                case (_roll < 95): {
                    _overcast = 0.85 + random 0.15;
                    _rain = 0.50 + random 0.30;
                    _fog = 0.08 + random 0.07;
                    _windMin = 8;
                    _windMax = 12;
                    _duration = 1500 + random 900;
                };
                ...
                default {
                    ...
                    _windMin = 12;
                    _windMax = 18;
                    _duration = 900 + random 600;
                };
            };

            private _dir = random 360;
            private _speed = _windMin + random (_windMax - _windMin);
			...
			0 setWaves (linearConversion [0, 18, _speed, 0.1, 1, true]);
