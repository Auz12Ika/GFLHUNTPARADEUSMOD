/*
    File: fn_triggerActivated_US.sqf
    Description: Spawner untuk trigger WEST (US Army, CULT sebagai OPFOR, MERC 50/50)
*/
params ["_trigger"];
...

{
    ...
    switch (_factionType) do {
        ...
        case "US": {
            _side = west;
            _unitPool = MERC_factions_USA;
            _vicPool = MERC_vehicles_USA;
		...
		if (_roll < 25) then {
			_unitPool = MERC_factions_SF;
			_vicPool = MERC_vehicles_SF;
			_mercType = "SF";
			_side = independent;
		}
		else {
			if (_roll < 50) then {
				_unitPool = MERC_factions_Mangi;
				_vicPool = MERC_vehicles_Mangi;
				_mercType = "MANGI";
				_side = selectRandom [independent, east, west];
			}
			else {
				if (_roll < 75) then {
					_unitPool = MERC_factions_Vanjager;
					_vicPool = MERC_vehicles_Vanjager;
					_mercType = "VANJAGER";
					_side = selectRandom [independent, east, west];
				}
				else {
