...
// ========================================================================
// FIX BUG #2: Client yang connect belakangan (JIP) TIDAK dapat broadcast
// awal di atas (target "0" cuma menjangkau client yang sudah online saat
// baris ini dieksekusi). Sediakan fungsi server yang bisa "diminta" oleh
// client kapan saja untuk menarik whitelist TERKINI secara langsung.
// ========================================================================
if (isNil "HQ_fnc_requestArsenalWhitelist") then {
    HQ_fnc_requestArsenalWhitelist = {
        params [["_requester", objNull, [objNull]]];
        if (isNull _requester) exitWith {};
        [MERC_HQ_ArsenalWhitelist] remoteExec ["HQ_fnc_HQwhitelist", _requester];
    };
};
    ...
    while {true} do {
        ...
        if (!isNull _hq) then {
			...
			if (_isUpdated) then {
				...
				{
					[_x] call HQ_fnc_requestArsenalWhitelist;
				} forEach allPlayers;
			};
