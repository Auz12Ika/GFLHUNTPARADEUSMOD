...
private _fnc_findSpawnPos = {
    ...
    while {!_found && _attempts < 100} do {
		...
		// 4. Validasi: tidak di tebing curam (perbedaan ketinggian > 5m)
		if (_found) then {
			// Pastikan _pos memiliki Z (3 elemen)
			if (count _pos < 3) then {
				_pos = [_pos select 0, _pos select 1, 0]; // tambahkan Z=0 jika kurang
			};
			private _height = _pos select 2;
			private _slopeOk = true;
			private _checkDirs = [0, 90, 180, 270];
			{
				private _checkPos = _pos getPos [10, _x];
				private _checkHeight = getTerrainHeightASL _checkPos;
				if (abs (_height - _checkHeight) > 5) then { _slopeOk = false; };
			} forEach _checkDirs;
			if (!_slopeOk) then { _found = false; };
		};
    ...
    if (_spawnPos isEqualTo [0,0,0]) then {
        ...
        if (_spawnPos isEqualTo [0,0,0]) then {
            ...
            while {_attempt < 50 && (_spawnPos isEqualTo [0,0,0] || surfaceIsWater _spawnPos)} do {
                ...
                if (count _flat > 0 && !surfaceIsWater _flat) then {
                    _spawnPos = _flat;
                };
            };
